...
# Quarterly Data Mapping Engine (Updated with the 4 specific income categories)
quarter_data = {
    "Q1": {
...
st.markdown("""
    ...
    * { font-family: 'Segoe UI', 'Roboto', 'Arial', sans-serif !important; box-sizing: border-box; }
    ...
    .kpi-card { 
        ...
        border-radius: 14px; 
        padding: 24px 20px; /* Increased internal padding */
        ...
        text-align: center; /* Perfect text center */
    ...
    .blue-card {
        background: #FFFFFF;
        border: 1px solid #E2E8F0;
        border-top: 4px solid #2563EB; /* Clean top accent */
        border-radius: 16px;
        padding: 20px;
    ...
    /* Streamlit Plotly Chart Cleanup */
...
with st.spinner("Rendering Visualizations..."):
    ...
    with sc1_left:
        ...
        fig_margin = go.Figure(data=[go.Pie(
            ...
            textposition='outside', # Prevents overlapping
            marker=dict(line=dict(color='#FFFFFF', width=2))
        )])
        fig_margin.update_layout(
            # Fixed perfectly centered annotation
            ...
            showlegend=False, 
            margin=dict(t=20, b=20, l=40, r=40), # Added left/right margin for outside labels
    ...
    with sc1_right:
        ...
        fig_trend = go.Figure()
        # Smooth line shapes to prevent weird drops
        fig_trend.add_trace(go.Scatter(x=q_ctx['months'], y=q_ctx['income_trend'], name='Total Income', line=dict(color=BLUE_ACCENT, width=4, shape='spline', smoothing=0.3), mode='lines+markers', marker=dict(size=10)))
        fig_trend.add_trace(go.Scatter(x=q_ctx['months'], y=q_ctx['expense_trend'], name='Total Expenses', line=dict(color=GREEN_ACCENT, width=4, shape='spline', smoothing=0.3), mode='lines+markers', marker=dict(size=10)))
        fig_trend.update_layout(
            ...
            # Fixed Y-Axis formatting (Removes scientific notation/micro units)
            yaxis=dict(
